...
sed -i -e s/name=\'jmbo-skeleton\'/name=\'${EGG}\'/ ${PROJECT_DIR}/setup.py
sed -i -e "s/PORT_PREFIX_PLACEHOLDER/${PORT}/g" ${PROJECT_DIR}/*.cfg
sed -i -e "s/SITE_NAME_PLACEHOLDER/${SITE}/g" ${PROJECT_DIR}/*.cfg
sed -i -e "s/PORT_PREFIX_PLACEHOLDER/${PORT}/g" ${PROJECT_DIR}/deviceproxy_*.yaml
sed -i -e "s/DEPLOY_TYPE_PLACEHOLDER/qa/g" ${PROJECT_DIR}/qa_*.cfg
sed -i -e "s/DEPLOY_TYPE_PLACEHOLDER/live/g" ${PROJECT_DIR}/live_*.cfg
sed -i -e "s/db_name/${APP}/g" ${PROJECT_DIR}/jenkins.sh

sed -i -e "14s/.*/    ${EGG}/" ${PROJECT_DIR}/dev_base.cfg

sed -i -e "16s/.*/    ${EGG}/" ${PROJECT_DIR}/qa_base_mobi.cfg
sed -i -e "17s/.*/    ${EGG}/" ${PROJECT_DIR}/qa_base_web.cfg
sed -i -e "16s/.*/    ${EGG}/" ${PROJECT_DIR}/qa_base_admin.cfg

sed -i -e "16s/.*/    ${EGG}/" ${PROJECT_DIR}/live_base_mobi.cfg
sed -i -e "17s/.*/    ${EGG}/" ${PROJECT_DIR}/live_base_web.cfg
sed -i -e "16s/.*/    ${EGG}/" ${PROJECT_DIR}/live_base_admin.cfg

# Replace the word skeleton with the app name
sed -i -e s/skeleton/${APP}/g ${PROJECT_DIR}/*.cfg
sed -i -e s/skeleton/${APP}/g ${PROJECT_DIR}/*.py
sed -i -e s/skeleton/${APP}/g ${APP_DIR}/*.py
sed -i -e s/skeleton/${APP}/g ${APP_DIR}/migrations/*.py

# Set the secret key
SECRET_KEY=`date +%s | shasum -a256 | head -c 56`
sed -i -e "s/SECRET_KEY_PLACEHOLDER/${SECRET_KEY}/" ${APP_DIR}/settings.py

# Replace site name in site specific buildout config files
if [ "$SITE" != "site" ];
then
    sed -i -e s/_site/_${SITE}/g ${PROJECT_DIR}/*_${SITE}.cfg
    sed -i -e s/-site/-${SITE}/g ${PROJECT_DIR}/*_${SITE}.cfg
fi
